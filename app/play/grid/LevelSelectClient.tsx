...
import { GridLevel, GRID_TOTAL_LEVELS } from "@/app/data/gridLevels";
...
export default function GridLevelSelectClient({ levels }: { levels: GridLevel[] }) {
  const [progress, setProgress] = useState<Record<string, boolean>>({});

  useEffect(() => {
    try {
      setProgress(JSON.parse(localStorage.getItem("csslab_grid_progress") ?? "{}"));
    } catch {}
  }, []);
  ...
  const pct = Math.round((totalCompleted / GRID_TOTAL_LEVELS) * 100);
  ...
  return (
    <div className="flex flex-col gap-10">
        ...
        <div className="flex gap-6">
          <div className="flex flex-col">
            <span className="text-2xl font-black" style={{ color: "var(--accent)" }}>
              {totalCompleted}
              <span className="text-base font-semibold" style={{ color: "var(--muted)" }}>/{GRID_TOTAL_LEVELS}</span>
            </span>
      ...
      {chapters.map((chIdx) => {
        ...
        const color = CHAPTER_COLORS[chIdx] ?? "#0ea5e9";
          ...
          <div key={chIdx}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {chLevels.map((level) => {
                ...
                return (
                  <Link key={level.id} href={`/play/grid/${level.id}`}
