...
import { GridLevel } from "@/app/data/gridLevels";
import { parseCSSInput, checkSolution, lockedCSSToString } from "@/app/lib/cssParser";

interface Props { level: GridLevel; totalLevels: number; }
    ...
    <div className="w-full rounded-xl" style={{
      ...
      {items.map((item, i) => (
        <div key={i} className="rounded-lg flex items-center justify-center font-bold"
          style={{
            minWidth: item.width ?? 48, minHeight: item.height ?? 52,
            background: item.bg, color: "#fff",
            fontSize: item.label.length > 1 ? 10 : 14,
            ...(itemCSS[i] as React.CSSProperties ?? {}),
          }}>
          {item.label}
        </div>
      ))}
        ...
        {Array.from({ length: total }, (_, i) => (
          <div key={i} className="rounded-full transition-all"
            style={{
              ...
              background: progress[start + i] ? "var(--accent)" : i + 1 === posInChapter ? "var(--accent-light)" : "var(--border)",
            }} />
        ))}
      </div>
    </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.88, opacity: 0, y: 24 }} animate={{ scale: 1, opacity: 1, y: 0 }}
        ...
        <h3 className="font-black text-2xl mb-1" style={{ color: "var(--accent)" }}>Level Cleared!</h3>
        {streak > 1 && (
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function GridGameClient({ level, totalLevels }: Props) {
  ...
  const previewContainerCSS = { ...level.lockedCSS, ...parsed };
  const previewItemCSS: Record<number, Record<string, string>> =
    level.editTarget === "container" ? {} : { [level.editTarget as number]: parsed };
  const targetItemCSS = level.targetItemCSS ?? {};
  const isLast = level.id === totalLevels;

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("csslab_grid_progress") ?? "{}");
      setProgress(saved);
      let s = 0;
  ...
  const handleCheck = useCallback(() => {
    ...
    if (ok) {
      ...
      confetti({ particleCount: 120, spread: 80, origin: { y: 0.6 }, colors: ["#0ea5e9", "#8b5cf6", "#10b981", "#f43f5e"] });
      try {
        const saved = JSON.parse(localStorage.getItem("csslab_grid_progress") ?? "{}");
        saved[level.id] = true;
        localStorage.setItem("csslab_grid_progress", JSON.stringify(saved));
        let s = 1;
        for (let i = level.id - 1; i >= 1; i--) { if (saved[i]) s++; else break; }
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            {lockedStr && (
              <div className="rounded-lg p-3 text-xs"
                style={{ background: "var(--code-bg)", border: "1px solid var(--border)", fontFamily: "var(--font-mono)", color: "var(--muted)", opacity: 0.7 }}>
          ...
          <AnimatePresence>
            {solved && (
              <SuccessOverlay
                ...
                onNext={() => { window.location.href = `/play/grid/${level.id + 1}`; }}
                isLast={isLast}
                streak={streak}
              />
            )}
        ...
        {showHint && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <p className="text-sm leading-relaxed mb-4 whitespace-pre-line" style={{ color: "var(--muted)" }}
                dangerouslySetInnerHTML={{
                  __html: level.hint.replace(/`(.*?)`/g,
                    '<code style="color:var(--accent);font-family:var(--font-mono);font-size:12px;background:var(--accent-dim);padding:1px 5px;border-radius:4px">$1</code>')
                }} />
