...
interface Props { level: Level; totalLevels: number; }
...
function FlexContainer({
  containerCSS, itemCSS = {}, items, height, showAxis,
}: {
  containerCSS: Record<string, string>;
  itemCSS?: Record<number, Record<string, string>>;
  items: Level["items"]; height: number; showAxis: boolean;
}) {
    ...
    <div className="relative w-full" style={{ minHeight: height }}>
      {showAxis && (
        <div className="absolute inset-0 pointer-events-none z-10">
          ...
          <span className="absolute font-mono font-bold px-1 rounded"
            style={{
              color: "var(--axis-main)", background: "var(--surface)",
              fontSize: 9, border: "1px solid var(--border-accent)",
          ...
          <span className="absolute font-mono font-bold px-1 rounded"
            style={{
              color: "var(--axis-cross)", background: "var(--surface)",
              fontSize: 9, border: "1px solid rgba(139,92,246,0.25)",
      ...
      <div className="w-full rounded-xl" style={{
        ...
        {items.map((item, i) => (
          <div key={i} className="rounded-lg flex items-center justify-center font-bold shrink-0"
            style={{
              width: item.width ?? 56, height: item.height ?? 56,
      ...
      <div className="flex items-center gap-1.5">
        {Array.from({ length: total }, (_, i) => (
          <div key={i} className="rounded-full transition-all"
            style={{
              width: i + 1 === posInChapter ? 20 : 8,
              height: 8,
              background: progress[start + i]
                ? "var(--accent)"
                : i + 1 === posInChapter
                  ? "var(--accent-light)"
                  : "var(--border)",
...
export default function GameClient({ level, totalLevels }: Props) {
  ...
  const previewContainerCSS: Record<string, string> = { ...level.lockedCSS, ...parsed };
  ...
  useEffect(() => {
    try {
      ...
      setProgress(saved);
      // Calculate streak ending at previous level
      let s = 0;
      for (let i = level.id - 1; i >= 1; i--) {
        if (saved[i]) s++; else break;
      }
      setStreak(s);
    } catch {}
  }, [level.id]);
  ...
  useEffect(() => {
    setUserInput(""); setSolved(false); setShowHint(false);
    setShake(false); setWrongAttempts(0);
  }, [level.id]);
  ...
  const handleCheck = useCallback(() => {
    ...
    if (ok) {
      ...
      playSuccess();
      // Confetti 🎉
      confetti({ particleCount: 120, spread: 80, origin: { y: 0.6 }, colors: ["#0ea5e9", "#38bdf8", "#8b5cf6", "#f43f5e", "#10b981"] });
      // Save progress
      try {
        const saved = JSON.parse(localStorage.getItem("flexlab_progress") ?? "{}");
        saved[level.id] = true;
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            {lockedStr && (
              <div className="rounded-lg p-3 text-xs" style={{
                ...
                fontFamily: "var(--font-mono)", color: "var(--muted)", opacity: 0.7,
              }}>
